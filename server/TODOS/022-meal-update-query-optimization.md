...
2.  Fetch the `Conversation` (using the meal's `conversation_id`).
...
## Required Fix
1.  **Update SQL Query**: Modify the `GetMealLog` SQL statement in `internal/db/queries/meal_logs.sql` to include a `LEFT JOIN conversations ON meal_logs.conversation_id = conversations.id`.
2.  **Add Join Columns**: Ensure the query selects `conversations.session_id` as part of the result.
...
4.  **Refactor Handler**: Update the `UpdateMeal` handler in `internal/routes/meals.go` to use the optimized retrieval, eliminating the separate `ConversationRepository.GetByID` call.

## Verification
- **CRITICAL**: Double-check that the `LEFT JOIN` correctly handles meals with `NULL` conversation IDs (e.g., legacy data or manually created logs).
