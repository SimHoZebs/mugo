...
func RegisterAgentEndpoints(humaAPI huma.API, prefix string, registry *adk.RunnerRegistry) {
	...
	huma.Register(agentsGroup, huma.Operation{
		...
		runner, ok := registry.Get("echo_agent")
		if !ok {
			return nil, fmt.Errorf("echo_agent not found")
		}

		result, err := runner.Run(ctx, input.Body.UserID, input.Body.SessionID, input.Body.Message)
		if err != nil {
	...
	huma.Register(agentsGroup, huma.Operation{
		...
		runner, ok := registry.Get("hello_time_agent")
		if !ok {
			return nil, fmt.Errorf("hello_time_agent not found")
		}

		result, err := runner.Run(ctx, input.Body.UserID, input.Body.SessionID, input.Body.City)
		if err != nil {
